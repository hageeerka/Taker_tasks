...
def show_timer(user_id):
    ...
    all_tasks[user_id][task_id]['timer'] = deadline - datetime.datetime.now()
    hours, remainder = divmod(all_tasks[user_id][task_id]['timer'].total_seconds(), 3600)
    minutes, seconds = divmod(remainder, 60)
    timer_string = "{:02}:{:02}:{:02}".format(int(hours), int(minutes), int(seconds))
...
def timer(user_id):
    deadline = datetime.datetime.strptime(all_tasks[user_id][task_id]['deadline'], "%Y-%m-%d %H:%M")
    all_tasks[user_id][task_id]['timer'] = deadline - datetime.datetime.now()
    while all_tasks[user_id][task_id]['timer'].total_seconds() > 0:
        hours, remainder = divmod(all_tasks[user_id][task_id]['timer'].total_seconds(), 3600)
        minutes, seconds = divmod(remainder, 60)
        timer_string = "{:02}:{:02}:{:02}".format(int(hours), int(minutes), int(seconds))
        all_tasks[user_id][task_id]['timer'] = timer_string
