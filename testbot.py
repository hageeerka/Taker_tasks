...
token = '6982284710:AAFSm5crO9AQm3WzgkXxz59MqCSR_iHt1jE'
...
def set_deadline(message):
    ...
    if all_tasks[task_id]['deadline'] is None or not fnmatch.fnmatch(message.text, "??.??.???? ??:??"):
        new_message = bot.send_message(chat_id, 'Пожалуйста, введите дедлайн по формату.')
        bot.register_next_step_handler(new_message, set_deadline)
        return
    else:
        bot.register_next_step_handler(message, edit_deadline)
        bot.send_message(chat_id, text='Выберите приоритет задачи.', reply_markup=markup)


def edit_deadline(message):
    chat_id = message.chat.id
    if not fnmatch.fnmatch(message.text, "??.??.???? ??:??"):
        bot.send_message(chat_id, 'Пожалуйста, введите дедлайн по формату.')
        bot.register_next_step_handler(message, edit_deadline)
        return
    else:
        all_tasks[task_id]['deadline'] = message.text.strip()
